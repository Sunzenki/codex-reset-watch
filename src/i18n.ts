...
export const currentCopy: Record<Locale, { headline: string; scope: string; context: string; note: string }> = {
  en: {
    ...
    scope: 'All ChatGPT Work and Codex users',
    context: 'Tibo said that all ChatGPT Work and Codex users have brand-new usage, explicitly describing the reset as completed.',
    note: 'The post did not give a separate execution time. Its timestamp records when the completion was publicly confirmed, not necessarily the exact moment every backend reset finished.',
  },
  'zh-CN': {
    headline: '新一轮 Codex 重置已落地',
    scope: '所有 ChatGPT Work 与 Codex 用户',
    context: 'Tibo 表示所有 ChatGPT Work 与 Codex 用户都获得了全新的使用额度，并明确将这次重置描述为已经完成。',
    note: '原帖没有另行给出后台执行时刻。本站以发帖时间记录公开确认时间，不将其表述为所有后台重置完成的精确秒点。',
  },
  'zh-TW': {
    headline: '新一輪 Codex 重置已落地',
    scope: '所有 ChatGPT Work 與 Codex 使用者',
    context: 'Tibo 表示所有 ChatGPT Work 與 Codex 使用者都獲得了全新的使用額度，並明確將這次重置描述為已經完成。',
    note: '原帖沒有另行提供後端執行時間。本站以發帖時間記錄公開確認時間，不將其表述為所有後端重置完成的精確秒點。',
  },
};
