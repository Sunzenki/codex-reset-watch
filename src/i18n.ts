...
export const ui = {
  en: {
    ...
    pushLabel: 'Browser reminders', pushTitle: 'Get notified about new resets', pushBody: 'No account required. Your browser stores a private subscription, and you can turn it off at any time.',
    pushEnable: 'Enable reminders', pushDisable: 'Turn off reminders', pushEnabled: 'Reminders are on for this browser.', pushDenied: 'Notifications are blocked in your browser settings.', pushUnsupported: 'This browser does not support CRW reminders yet.', pushWorking: 'Please wait…', pushError: 'Could not update the reminder. Please try again.',
  ...
  'zh-CN': {
    ...
    pushLabel: 'Browser reminders', pushTitle: '额度重置浏览器提醒', pushBody: '无需注册。订阅只绑定当前浏览器，可随时关闭；仅在出现新的额度重置消息时推送。',
    pushEnable: '开启提醒', pushDisable: '关闭提醒', pushEnabled: '此浏览器已开启提醒。', pushDenied: '浏览器已阻止通知，请在网站权限中重新开启。', pushUnsupported: '当前浏览器暂不支持 CRW 提醒。', pushWorking: '处理中…', pushError: '提醒设置失败，请稍后重试。',
  ...
  'zh-TW': {
    ...
    pushLabel: 'Browser reminders', pushTitle: '額度重置瀏覽器提醒', pushBody: '無需註冊。訂閱只綁定目前瀏覽器，可隨時關閉；僅在出現新的額度重置消息時推送。',
    pushEnable: '開啟提醒', pushDisable: '關閉提醒', pushEnabled: '此瀏覽器已開啟提醒。', pushDenied: '瀏覽器已封鎖通知，請在網站權限中重新開啟。', pushUnsupported: '目前瀏覽器暫不支援 CRW 提醒。', pushWorking: '處理中…', pushError: '提醒設定失敗，請稍後再試。',
...
export const currentCopy: Record<Locale, { headline: string; scope: string; context: string; note: string }> = {
  "en": {
    "headline": "A new usage reset announced",
    "scope": "All paid Codex and ChatGPT Work users will receive a usage reset as compensation for the outage.",
    "context": "Tibo says service is back and promises a usage-limit reset for all paid users across Codex and ChatGPT Work, alongside an apology for the brief disruption.",
    "note": "“Compensation” is CRW’s summary of the outage, apology, and reset announcement. The post does not confirm completion or specify whether the reset is automatic or banked. The post timestamp is not an execution time."
  },
  "zh-CN": {
    "headline": "新一轮额度重置已宣布",
    "scope": "作为服务中断补偿，Codex 与 ChatGPT Work 的所有付费用户将获额度重置。",
    "context": "Tibo 表示服务已恢复，将为 Codex 和 ChatGPT Work 的所有付费用户重置使用额度，并为此次短暂中断致歉。",
    "note": "“补偿”是本站结合服务中断、致歉和重置承诺作出的概括。原帖未确认重置已完成，也未说明是自动重置还是储备重置；发帖时间不代表执行时间。"
  },
  "zh-TW": {
    "headline": "新一輪額度重置已宣布",
    "scope": "作為服務中斷補償，Codex 與 ChatGPT Work 的所有付費使用者將獲額度重置。",
    "context": "Tibo 表示服務已恢復，將為 Codex 和 ChatGPT Work 的所有付費使用者重置使用額度，並為此次短暫中斷致歉。",
    "note": "「補償」是本站結合服務中斷、致歉和重置承諾作出的概括。原帖未確認重置已完成，也未說明是自動重置還是儲備重置；發帖時間不代表執行時間。"
  }
};
...
export function localizedSourceTimezone(value: string | null, locale: Locale) {
  if (value === "Not provided; X timestamp recorded in UTC") return locale === "en" ? value : locale === "zh-TW" ? "原帖未註明；X 時間戳以 UTC 記錄" : "原帖未注明；X 时间戳以 UTC 记录";
