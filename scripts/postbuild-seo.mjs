...
const locales = {
  en: {
    ...
    rolloutHeadline: 'Plus 5-hour limit rollout has begun', confirmedResetHeadline: 'A fresh Codex usage reset has landed',
  ...
  'zh-CN': {
    ...
    rolloutHeadline: 'Plus 5 小时限制已开始落地', confirmedResetHeadline: '新一轮 Codex 重置已落地',
  },
  'zh-TW': {
    ...
    rolloutHeadline: 'Plus 5 小時限制已開始落地', confirmedResetHeadline: '新一輪 Codex 重置已落地',
  },
};
...
function structuredData(route) {
  ...
  if (route.kind === 'home' && current.announcement) {
    ...
    webpage.mainEntity = {
      '@type': 'CreativeWork', name: current.kind === 'reset_confirmed' ? 'Latest confirmed Codex usage reset' : current.kind === 'rollout_observed' ? 'Latest Codex rate-limit rollout update' : 'Current Codex reset estimate',
