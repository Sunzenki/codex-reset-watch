...
function structuredData(route) {
  ...
  if (route.kind === 'home' && current.announcement) {
    ...
    webpage.mainEntity = {
      '@type': 'CreativeWork', name: current.kind === 'reset_confirmed' ? 'Latest confirmed Codex usage reset' : current.kind === 'rollout_observed' ? 'Latest Codex rate-limit rollout update' : current.status === 'monitoring' && !current.resetAt ? 'Latest possible Codex reset hint' : 'Current Codex reset estimate',
