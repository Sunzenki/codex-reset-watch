...
const seoMetadata = JSON.parse(readFileSync(join(ROOT, 'src/seo-metadata.json'), 'utf8'));
...
const locales = {
  en: {
    ...
    expected: 'Reset expected around', bankedExpected: 'Banked reset expected around', bankedReached: 'Expected banked-reset availability time reached', bankedPending: 'Tibo agreed in a reply to provide one banked reset', reached: 'The announced reset time has passed',
  ...
  'zh-CN': {
    ...
    expected: '预计', bankedExpected: '储备重置机会预计于', bankedReached: '储备重置机会的预计到账时间已到', bankedPending: 'Tibo 在回复中答应一次储备重置', reached: '预告重置时间已到', source: '公开原始来源', updated: '本站人工更新', historyIntro: '过往公开重置消息及其时间换算记录。',
  ...
  'zh-TW': {
    ...
    expected: '預計', bankedExpected: '儲備重置機會預計於', bankedReached: '儲備重置機會的預計到帳時間已到', bankedPending: 'Tibo 在回覆中答應一次儲備重置', reached: '預告重置時間已到', source: '公開原始來源', updated: '本站人工更新', historyIntro: '過往公開重置消息及其時間換算記錄。',
...
function confirmedHeadline(locale) {
  if (current.confirmationBasis === 'owner_observed') return locales[locale].confirmedResetHeadline;
...
function currentHeadline(locale) {
  ...
  if (current.kind === 'banked_reset') {
    ...
    return locale === 'en' ? `${copy.bankedExpected} ${time}` : `${copy.bankedExpected} ${time} ${locale === 'zh-TW' ? '左右到帳' : '左右到账'}`;
  }
  if (current.kind === 'reset' && current.status === 'monitoring' && !current.resetAt && current.announcement) return copy.untimedHintHeadline;
...
function snapshot(route) {
  ...
  return `<main id="content" class="seo-snapshot"><header><p>Codex Reset Watch</p><h1>${escapeHtml(copy.historyTitle)}</h1><p>${escapeHtml(copy.historyDescription)}</p></header><ol>${items}</ol><nav aria-label="Languages">${languageLinks}</nav></main>`;
}
...
function structuredData(route) {
  ...
  const website = {
    '@type': 'WebSite', '@id': `${ORIGIN}/#website`, url: `${ORIGIN}/`, name: 'Codex Reset Watch',
    description: seoMetadata.en.home.description,
  ...
  const webpage = {
    ...
    name: route.kind === 'history' ? copy.historyTitle : copy.homeTitle,
    description: route.kind === 'history' ? copy.historyDescription : copy.homeDescription,
